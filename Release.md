...
-	**Generated Dockerfile (Docker Official Images compliance)**  
	Generated Dockerfiles: (1) do **not** include `# syntax=docker/dockerfile:1` — that BuildKit parser directive is explicitly omitted for Docker Official Images (DOI), whose legacy builder does not use BuildKit; (2) start with a single blank line followed by the standard Aerospike comment header and `FROM`; (3) inline all install logic as a `RUN \` continuation block (no heredocs, no `COPY install.sh`) — DOI's parser cannot pre-scan BuildKit heredocs, and `install.sh` is not part of the bashbrew build context; (4) use `dpkg --print-architecture` or `rpm --eval '%{_arch}'` for userspace architecture detection (not `uname -m`, which reports the kernel/host arch); (5) have trailing whitespace stripped and are written with a trailing newline.

-	**scripts/deb/install.sh — distro-aware compat libs**

	-	**Ubuntu 22.04 (Jammy):** Use Focal-only compat repo (no Jammy). Adding Jammy made apt treat `libldap-2.4-2` as obsolete ("replaced by libldap-common") and broke the install. On **Jammy arm64** only, skip the apt path and use direct .deb download + `dpkg -i` to avoid QEMU libc-bin postinst segfault (exit 139) when cross-building arm64 on amd64.  
	-	**Ubuntu 24.04 (Noble, 8.1):** Keep Focal+Jammy compat repos and normal apt path so 8.1 continues to work.  
	-	**General:** Early `dpkg --configure -a` (and on arm64 a retry after sleep). In `install_compat_libs`, on arm64 use `dpkg -i ... || true` and `dpkg --configure -a` (no `apt-get install -f -y`) to avoid triggering libc-bin under QEMU. Version detected from `/etc/os-release` (`VERSION_ID`).

-	**Architecture filter (-a, --arch)**
