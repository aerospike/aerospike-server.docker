...
RUN \
  ...
  { \
    ...
    ls -lat /tmp; \
    /tmp/get-artifacts.sh git@github.com:citrusleaf/aerospike-server arm-ubuntu-24.04 cdt-key-list enterprise; \
    mv artifacts/aerospike*.deb aerospike-server.deb; \

    # get latest nightly tools deb
    /tmp/get-artifacts.sh git@github.com:citrusleaf/aerospike-tools arm-ubuntu-24.04 master default; \
