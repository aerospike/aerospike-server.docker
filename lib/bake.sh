    ...
    for lineage in "${LINEAGES_TO_BUILD[@]}"; do
        ...
        for edition in $(support_editions); do
            ...
            for distro in ${distros_building}; do
                ...
                # Directory existence is not enough: a target this run skipped
                # still has its committed Dockerfile, pinned to whatever version
                # it was last generated for, and the tags below carry the version
                # resolved this run. Building it would publish the previous
                # build's contents under the new version's tags.
                local _target="${ctx#./}" _generated=false _t
                for _t in "${G_GENERATED_TARGETS[@]:-}"; do
                    [ "${_t}" = "${_target}" ] && {
                        _generated=true
                        break
                    }
                ...
                if [ "${_generated}" = false ]; then
                    log_warn "Omitting ${_target} from the bake file - not generated by this run"
                    continue
                fi
